...
function [u_opt,x0_opt,lam_opt,phi_opt,psi_opt,F_opt] =...
    direct_multiple_shooting(t0,tf,N,cvode,lbu,ubu,u0,lbx,ubx,x0,t,x,u,p,pm,xdot,qdot,phi,psi,lam0)
...
w_opt = reshape([w_opt;nan*ones(nu,1)],nx+nu,[]);
u_opt = w_opt((nx+1):end,:);
x0_opt = x0(:,1);
lam_w_opt = reshape([lam_w_opt;nan*ones(nu,1)],nx+nu,[]);
