...
function [tf_opt,u_opt,x0_opt,lam_opt,phi_opt,psi_opt,F_opt] =...
    direct_multiple_shooting(t0,tf,N,cvode,lbu,ubu,tf0,u0,lbx,ubx,x0,t,x,u,p,pm,xdot,qdot,phi,psi,lam0)
...
for k = 1:N
    Fk = F('x0',x0(:,k),'p',[pm;u0(:,k);(tf0-t0)/N]);
    x0(:,k+1) = full(Fk.xf);
end
...
if(~isempty(lam0))
    lam_w0 = [lam_w0;reshape([lam_x0;lam_u0],[],1)];
    lam_w0 = lam_w0(1:(end-nu));
    lam_g0 = [lam_g0;lam_dx0(:)];
end
...
w_opt = reshape([w_opt((flagt+1):end);nan*ones(nu,1)],nx+nu,[]);
u_opt = w_opt((nx+1):end,:);
x0_opt = x0(:,1);
lam_tf_opt = lam_w_opt(1:flagt);
lam_w_opt = reshape([lam_w_opt((flagt+1):end);nan*ones(nu,1)],nx+nu,[]);
lam_u_opt = lam_w_opt((nx+1):end,:);
lam_dt_opt = lam_g_opt(1);
lam_x_opt = lam_w_opt(1:nx,:);
