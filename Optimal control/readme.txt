...
- The code file test_hessian_appj.m provides the results that would be obtained if the true APPJ were perfectly known in Section IV of the paper "Data-Driven Adaptive Optimal Control Under Model Uncertainty: An Application to Cold Atmospheric Plasmas". The code file test_appj_track.m provides the results of an adaptive approach in the presence of plant-model mismatch and disturbances for the APPJ in Section IV of the paper "Data-Driven Adaptive Optimal Control Under Model Uncertainty: An Application to Cold Atmospheric Plasmas". The code file test_casadi_appj.m uses CasADi to provide the results of model predictive control via a parsimonious parameterization approach, single shooting, and multiple shooting for the APPJ in Section IV of the paper "Data-Driven Adaptive Optimal Control Under Model Uncertainty: An Application to Cold Atmospheric Plasmas".
- The code files test_hessian_pyrrole_bc_analyt.m and test_hessian_goddard_analyt.m are similar to the files test_hessian_pyrrole_bc.m and test_hessian_goddard.m, respectively, but the former ones use analytical expressions for the input in the free/singular arcs, while the latter ones use approximation of the input in the free/singular arcs by linear functions of time.
